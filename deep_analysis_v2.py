#!/usr/bin/env python3
"""
Deep Analysis Test Suite for MetroCoop Web Application - CORRECTED VERSION
Based on actual login page structure discovered via debug
"""
...
# Test credentials - using the quick-login buttons
QUICK_LOGINS = {
    "admin": {"button_text": "🔑 Admin / Pengurus", "tab": "Admin & Operator"},
    "operator": {"button_text": "⚙️ Operator Staff", "tab": "Admin & Operator"},
    "anggota": {"button_text": "👤 Anggota", "tab": "Portal Anggota"},
    "perusahaan": {"tab": "Perusahaan", "username": "hijau_agri", "password": "perusahaan123"},  # No quick login, manual
}
...
def login_via_quick_button(page, role):
    """Login using the quick-login credential buttons"""
    creds = QUICK_LOGINS[role]
    print(f"\n🔐 Logging in as {role} via quick button...")
    
    page.goto(f"{BASE_URL}/login")
    page.wait_for_load_state('networkidle')
    page.wait_for_timeout(1000)
    
    # Click the quick-login button (it auto-fills credentials)
    if "button_text" in creds:
        quick_btn = page.locator(f'button:has-text("{creds["button_text"].split(chr(10))[0]}")').first
        if quick_btn.count() > 0:
            quick_btn.click()
            print(f"  ✅ Clicked quick-login button")
            page.wait_for_timeout(1000)
            
            # Now click the submit button
            submit_btn = page.locator('button:has-text("Masuk Sistem")').first
        ...
        # Manual login for perusahaan
        return login_manual(page, role)

def login_manual(page, role):
    """Manual login for roles without quick button"""
    creds = QUICK_LOGINS[role]
    print(f"\n🔐 Logging in as {role} manually...")
    ...
    # Submit
    page.click('button:has-text("Masuk Sistem")')
    page.wait_for_load_state('networkidle')
    page.wait_for_timeout(2000)
...
def test_module(page, module_name):
    ...
    try:
        ...
        if btn.count() > 0 and btn.is_visible():
            ...
            for sel in ['table', '.card', '[class*="card"]', 'form', '[class*="grid"]', '[class*="list"]', '.content', 'text=Data', 'text=Daftar', 'text=Tabel', 'text=Statistik', 'text=Total', 'text=Saldo']:
                if page.locator(sel).count() > 0:
                    content_loaded = True
                    break
...
def run_deep_analysis():
    ...
    with sync_playwright() as p:
        ...
        for role in ["admin", "operator", "anggota", "perusahaan"]:
            ...
            results["login_tests"][role] = {
                ...
                "method": "quick_button" if role != "perusahaan" else "manual"
            }
    ...
    for role, data in results["login_tests"].items():
        print(f"  {'✅' if data['success'] else '❌'} {role} ({data['method']})")
    ...
    for e in results["errors_found"]:
        print(f"  - {e.get('role', e.get('module', '?'))}: {e.get('error', '?')}")
