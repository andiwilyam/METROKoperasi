...
CREATE TABLE IF NOT EXISTS venture_investments (
  ...
  perusahaan_id_fk VARCHAR(50) DEFAULT NULL
);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'venture_investments_pengajuan_id_fkey'
  ) THEN
    ALTER TABLE venture_investments
      ADD CONSTRAINT venture_investments_pengajuan_id_fkey
      FOREIGN KEY (pengajuan_id) REFERENCES pengajuan_pembiayaan(id);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'venture_investments_perusahaan_id_fk_fkey'
  ) THEN
    ALTER TABLE venture_investments
      ADD CONSTRAINT venture_investments_perusahaan_id_fk_fkey
      FOREIGN KEY (perusahaan_id_fk) REFERENCES perusahaan(id);
  END IF;
END $$;
