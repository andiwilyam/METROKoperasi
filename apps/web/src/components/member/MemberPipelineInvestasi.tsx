...
import { TrendingUp, Building2, DollarSign, Calendar, CheckCircle, Clock, X, ArrowUpRight, HelpCircle, ShieldCheck, Coins, FileText } from 'lucide-react';
import { useDataStore } from '../../stores/dataStore';

export default function MemberPipelineInvestasi({ session }: { session: any }) {
  const { investments, fetchInvestments, pengajuanList, fetchPengajuan } = useDataStore();
  ...
  // Get company's data
  const myInvestments = investments.filter((i: any) => 
    i.perusahaanId === session.perusahaanId || i.perusahaanId === session.id
  );
  const myPengajuan = pengajuanList.filter((p: any) => 
    p.perusahaanId === session.perusahaanId || p.perusahaanId === session.id
  );

  // Categorize pengajuan
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {myPengajuan.map((p: any) => (
                    <div
                      key={p.id}
                      className="border border-slate-200 rounded-lg p-4 hover:bg-slate-50/50 transition flex flex-col md:flex-row md:items-center md:justify-between gap-4 cursor-pointer"
