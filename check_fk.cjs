...
(async () => {
  try {
    const res = await pool.query("SELECT conname, confrelid::regclass, conrelid::regclass FROM pg_constraint WHERE conrelid = 'pengajuan_pembiayaan'::regclass AND contype = 'f'");
    console.log('FK on pengajuan_pembiayaan:', res.rows);
    await pool.end();
